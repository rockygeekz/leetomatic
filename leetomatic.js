...
const fetchLeetCodeDailyProblem = async () => {
  ...
  try {
    ...
    console.log(
      `Problem Fetched: ${title} - https://leetcode.com/problems/${titleSlug}/`
    );
    return titleSlug;
  } catch (error) {
...
const searchSolutionFile = async (problemTitle) => {
  ...
  try {
    ...
    if (!file) {
      throw new Error(
        `No relevant solution file found for problem: ${problemTitle}`
      );
    }
...
const submitToLeetCode = async (code, leetcodeUrl) => {
  ...
  try {
    ...
    // Use persistent context to store cookies and session data
    // Use persistent context to store cookies and session data
    const USER_DATA_DIR = path.join(process.cwd(), "user-data-dir");
...
(async () => {
  try {
    ...
    if (!isValidSession) {
      console.error(
        "Session is invalid. Please update the SESSION_COOKIE in GitHub Secrets."
      );
      process.exit(1);
    }
    ...
    const leetcodeUrl = `https://leetcode.com/problems/${encodeURIComponent(
      problemSlug
    )}/`;
